...
features_player  = features_past_generation(features_player_creation,5,"playerft5",data,indices) #is this supposed to be 5?
...
early_stop=[5]
subsample=[0.8]
params=np.array(np.meshgrid(learning_rate,max_depth,min_child_weight,gamma,csbt,lambd,alpha,num_rounds,early_stop,subsample)).T.reshape(-1,10).astype(np.float64)
...
test_beginning_match=data[data.Date==start_date].index[0] #id of the first match of the testing set
span_matches=len(data)-test_beginning_match+1
duration_val_matches=1000
duration_train_matches=8000
duration_test_matches=1000
...
plotProfits(conf,"Test on the period Jan. 2019 -> Jan 2023")
